...
gen_iptables_sock() {
    cat <<EOF
    $(awk -F "/" '{print "iptables -I INPUT -p tcp --dport " $4 "  -m state --state NEW -j ACCEPT"}' ${SOCKDATA}) 
EOF
}
...
gen_ifconfig_sock() {
    cat <<EOF
$(awk -F "/" '{print "ifconfig enp1s0 inet6 add " $5 "/64"}' ${SOCKDATA})
EOF
}
...
LAST_SOCK_PORT=62000

gen_data >$WORKDIR/data.txt
gen_sock_data >$WORKDIR/sock.txt
gen_iptables >$WORKDIR/boot_iptables.sh
gen_iptables_sock > $WORKDIR/boot_iptables_sock.sh
gen_ifconfig >$WORKDIR/boot_ifconfig.sh
gen_ifconfig_sock >$WORKDIR/boot_ifconfig_sock.sh
...
bash ${WORKDIR}/boot_iptables_sock.sh
bash ${WORKDIR}/boot_ifconfig.sh
bash ${WORKDIR}/boot_ifconfig_sock.sh
ulimit -n 65535
